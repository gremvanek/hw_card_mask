...
@pytest.mark.parametrize("card_mask_test, expected", [("15968705199", "Введенные данные неверные"),
                                                      ("715830726758", "Введенные данные неверные"),
                                                      ("7000792289606361", "7000 79** **** 6361")])
def test_mask_card(card_mask_test: str, expected):
    assert mask_card(card_mask_test) == expected


@pytest.fixture
def check_out_mask_test():
    return ["64686473", "353830334789556", "73654108430135874305"]


@pytest.mark.parametrize("check_out_mask_test, expected", [("64686473", "Введенные данные неверные"),
                                                           ("353830334789556", "Введенные данные неверные"),
                                                           ("73654108430135874305", "**4305")])
def test_check_mask(check_out_mask_test: str, expected):
